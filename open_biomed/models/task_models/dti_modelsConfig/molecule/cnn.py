...
from open_biomed.models.task_models.dti_modelsConfig.base_models import MolEncoder
...
class MolCNN(MolEncoder):
    ...
    def _get_conv_output(self, shape):
        ...
        print(f"[Debug] 输入形状: {input.shape}")  # 应为 (1, 63, 100)
        output_feat = self._forward_features(input.double())
        print(f"[Debug] 卷积池化后形状: {output_feat.shape}")  # 应为 (1, 96, 1)
        n_size = output_feat.data.view(bs, -1).size(1)
        print(f"[Debug] 展平后特征数: {n_size}")  # 应为 96
        return n_size
